...
\subsubsection{$\sqrt{\pm\frac{x-a}{x-b}}$ and $\sqrt{(x-a)(x-b)}$}
...
\item $ \int \cos^m x \sin^n x \md x\\
	= \frac{1}{m+n} \cos^{m-1} x \sin^{n+1}x + \frac{m-1}{m+n}\int\cos^{m-2}x\sin^nx\md x\\
	= -\frac{1}{m+n} \cos^{m+1} x \sin^{n-1}x + \frac{n-1}{m+1} \int \cos^m x\sin^{n-2} x \md x $
...
\subsubsection{Integration of inverse trigonometric functions ($a>0$)}
...
\subsubsection{Integration of exponential functions}
...
\subsubsection{Integration of logistic functions}
